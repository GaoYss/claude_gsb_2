...
class GreenSpace(TimestampMixin, db.Model):
    ...
    soil_tests = db.relationship(
        "SoilTest", back_populates="green_space", cascade="all, delete-orphan"
    )
    fertilizations = db.relationship(
        "FertilizationRecord", back_populates="green_space", cascade="all, delete-orphan"
    )
