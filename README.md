...
面向城市绿化养护单位的一体化记录系统：以**绿地台账**为主线，串联**养护任务登记**、**养护记录录入**、**绿植更换记录**与**土壤检测档案（施肥配方与施肥回填）**，并提供养护总览看板。
...
| 土壤检测 | `/soil-tests` | 检测档案登记（采样日期、点位、检测机构、pH、有机质、碱解氮/有效磷/速效钾与丰缺评级）、结合绿地类型与目标作物给出施肥配方、施肥作业引用配方回填实际用量、同一绿地历次检测结果对比 |
...
│   │   │   ├── common.py        # 链式字段校验器
│   │   │   ├── filters.py       # 列表过滤条件
│   │   │   └── green_space.py / maintenance_task.py / maintenance_record.py / plant_replacement.py / soil_test.py
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、20 条任务、26 条养护记录、11 条更换记录、8 次土壤检测、13 条施肥作业）。停止与清理：
...
| GET | `/meta/soil-indicators` | 土壤检测指标名称、单位与丰缺评级分界 |
...
| GET/POST | `/soil-tests` | 土壤检测档案列表（`green_space_id`/`lab`/`keyword`/采样日期区间，返回指标均值汇总） / 登记检测（含嵌套施肥配方） |
| GET/PUT/DELETE | `/soil-tests/{id}` | 档案详情（含配方明细与各配方执行次数、累计实际用量） / 更新（全量同步配方） / 删除（施肥作业保留履历并解除引用） |
| GET | `/soil-tests/summary` | 检测汇总（份数与各指标平均值） |
| GET | `/soil-tests/comparison` | 同一绿地历次检测对比（各指标值、等级、与上次差值） |
| GET | `/soil-tests/formula-options` | 某绿地最近检测档案及配方，供施肥作业引用 |
| GET/POST | `/fertilizations` | 施肥作业列表（`green_space_id`/`soil_test_id`/`method`/日期区间，返回汇总与配方引用率） / 登记施肥（引用配方回填实际用量） |
| GET/PUT/DELETE | `/fertilizations/{id}` | 详情 / 更新 / 删除 |
| GET | `/fertilizations/summary` | 施肥汇总（次数、建议/实际用量合计、配方执行率） |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口 |

## 六、业务规则

1. **业务编号**：绿地 `GS-年份-序号`（如 `GS-2026-0001`），任务 `MT-YYYYMMDD-序号`，养护记录 `MR-YYYYMMDD-序号`，更换记录 `PR-YYYYMMDD-序号`，土壤检测 `ST-YYYYMMDD-序号`，施肥作业 `FT-YYYYMMDD-序号`；留空自动生成，唯一约束冲突时自动重试，编号创建后不可修改。
...
6. **删除保护**：删除绿地时若已存在任务/记录/更换/检测/施肥数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务时养护记录默认保留（解除关联），避免养护履历丢失。删除检测档案时其配方明细一并清除，施肥作业保留履历并解除对档案与配方的引用。
7. **字典单一来源**：所有枚举在 `backend/app/constants.py` 定义，前端通过 `/meta/enums` 获取并缓存，前后端不重复维护。
8. **土壤检测与施肥配方**：
   - 检测编号 `ST-YYYYMMDD-序号`、施肥作业编号 `FT-YYYYMMDD-序号`，留空自动生成；
   - pH、有机质、碱解氮、有效磷、速效钾按五档阈值自动评级（极低/偏低/适中/偏高/过高，阈值见 `utils/soil.py`，经 `/meta/soil-indicators` 下发）；
   - 未填写施肥建议时，后端结合**绿地类型与目标作物**及各指标丰缺等级自动生成改土施肥建议；
   - 一次检测可挂多条施肥配方（肥料、养分配比、建议用量/单位、施肥方式、施用时期），更新检测时按 `id` 全量同步；
   - 登记施肥作业可引用配方并自动快照肥料名称、建议用量，再回填**实际用量**；配方必须属于同一绿地，养护记录关联同样校验绿地一致；
   - `/soil-tests/comparison` 返回同一绿地历次检测的指标值、等级及与上一次的差值，支撑改土效果对比。
...
| `soil_test` | 土壤检测档案 | `test_no`(唯一)、`green_space_id`、`sample_date`、`sample_point`、`sample_depth`、`lab`、`lab_report_no`、`texture`、`ph`、`organic_matter`、`alkaline_n`、`available_p`、`available_k`、`target_crop`、`fert_advice` |
| `soil_formula_item` | 施肥配方明细 | `soil_test_id`、`product_name`、`nutrient_ratio`、`dose`、`dose_unit`、`method`、`timing` |
| `fertilization_record` | 施肥作业 | `fert_no`(唯一)、`green_space_id`、`soil_test_id`(可空)、`formula_item_id`(可空)、`maintenance_record_id`(可空)、`product_name`、`planned_dose`、`actual_dose`、`actual_unit`、`fert_date` |

绿地删除时任务/记录/更换/检测/施肥级联清理；任务与养护记录之间、养护记录与更换记录之间为可空外键（`SET NULL`），保证养护履历可独立留存。检测档案删除时施肥作业保留，对档案与配方的引用置空，肥料名称与用量以快照形式留在作业记录中。

## 八、测试

```bash
cd backend
python -m pytest              # 74 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、删除保护与强制删除、土壤指标丰缺评级与自动建议、嵌套配方全量同步、施肥引用配方回填实际用量、同绿地历次检测对比、检测删除后施肥履历保留、统计聚合口径一致性、演示数据自洽性。
